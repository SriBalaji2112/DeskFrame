...
class CreateDialog:
    def __init__(self, name):
        directory_ = os.getcwd()
        python_dir = os.path.join(directory_, "python")
        dialog_dir = os.path.join(python_dir, "dialog")
        res_dir = os.path.join(directory_, "res")
        layout_dir = os.path.join(res_dir, "layout")
        dialog_layout_dir = os.path.join(layout_dir, "dialog")
        activity_name, layout_name = format_activity_name(name)

        if not os.path.exists(dialog_dir):
            os.makedirs(dialog_dir)
            os.makedirs(dialog_layout_dir)
            colorPrint(Fore.RED, "[INFO] Dialog Window Created...")

        if os.path.exists(dialog_dir + "/" + activity_name + ".py"):
            colorPrint(Fore.RED, f"Activity is already exists using this name..")
            exit(1)
...
def main():
    ascii_pydesk2 = asciiPrint("P Y D E S K 2")
    colorPrint(Fore.GREEN, ascii_pydesk2)
    ...
    parser.add_argument("--createDialog", type=str, help="Create an Dialog window with the specified name.")
